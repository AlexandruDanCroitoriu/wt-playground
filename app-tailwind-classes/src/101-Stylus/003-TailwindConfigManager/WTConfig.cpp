...
namespace Stylus
{
        ...
        : default_config_path_(default_config_path)
    {
        ...
        auto write_config_btn = buttons_wrapper->addWidget(std::make_unique<Wt::WPushButton>("Write Config"));
        write_config_btn->setStyleClass("btn-default");
        ...
        write_config_btn->clicked().connect(this, [=](){ writeConfig(); });
    ...
    void WTConfig::createVariableWidgetsV2()
    {
        vars_wrapper_->clear();

        std::string title_styles = "text-2xl font-bold text-gray-800 dark:text-gray-200";
        std::string content_styles = "flex flex-col bg-white dark:bg-gray-800 dark:text-gray-200";

        Wt::WContainerWidget *panel = vars_wrapper_->addWidget(std::make_unique<Wt::WContainerWidget>());
        panel->addWidget(std::make_unique<Wt::WText>("Font Family"))->setStyleClass(title_styles);
        Wt::WContainerWidget *content = panel->addWidget(std::make_unique<Wt::WContainerWidget>());
        content->setStyleClass(content_styles + " max-w-[1000px]");

        for (auto &tVar : var_font_family_)
        {
            auto wTVariable = content->addWidget(std::make_unique<WTVariable>(tVar.second));
            if (default_config_file_.compare(selected_config_file_) == 0)
                wTVariable->set_read_only();
        }
        panel = vars_wrapper_->addWidget(std::make_unique<Wt::WContainerWidget>());
        content = panel->addWidget(std::make_unique<Wt::WContainerWidget>());
